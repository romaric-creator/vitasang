...
const seedDatabase = async () => {
  ...
  try {
    ...
    await Utilisateur.destroy({
      where: { nom: { [db.Sequelize.Op.like]: "%(Test)" } },
    });
    ...
    for (let i = 0; i < NUM_DONORS; i++) {
      ...
      donorsToCreate.push({
        user: {
          nom: `${lastName} (Test)`,
          prenom: firstName,
          // Format camerounais: +237 suivi de 6 ou 2, puis 8 chiffres
          telephone: `+237${faker.helpers.arrayElement(["6", "2"])}${faker.string.numeric(8)}`,
        ...
        profile: {
          ...
          lat_actuelle: faker.location.latitude({
            min: 48.7,
            max: 49.0,
            precision: 6,
          }),
          long_actuelle: faker.location.longitude({
            min: 2.2,
            max: 2.5,
            precision: 6,
          }),
