export default {
  common: {
    seeAll: "See all",
    cancel: "Cancel",
    confirm: "Confirm",
    error: "Error",
    success: "Success",
    bloodGroup: "Blood group",
    loading: "Loading...",
    notProvided: "Not provided",
    unknown: "Unknown",
    errors: {
      oops: "Oops! An error occurred",
      unexpected: "An unexpected error occurred.",
      retry: "Retry",
      ok: "OK",
      error: "Error",
      warning: "Warning",
      info: "Information",
    },
    actions: {
      skip: "Skip",
      next: "Next",
      start: "Start",
      backHome: "BACK TO HOME",
      findCenter: "FIND A CENTER",
      retry: "RETRY",
      yes: "YES",
      no: "NO",
    },
    compatible: "Compatible",
  },
  error: {
    network: "Network error. Please check your internet connection.",
    invalidCredentials: "Incorrect phone number or password.",
  },
  tabs: {
    home: "Home",
    map: "Centers",
    alerts: "Alerts",
    profile: "Profile",
    messages: "Messages",
  },
  home: {
    tagline: "Saving lives together",
    profileLabel: "HELLO",
    alertsLabel: "Alerts",
    launchAlert: "Launch an Alert",
    urgentNeed: "Urgent blood donation • Needed now",
    urgentSection: "Urgent Needs",
    donate: "Donate",
    respond: "Respond",
    myStatus: "My Status",
    nextDonation: "Next donation",
    livesSaved: "Lives saved",
    tips: "Help & Advice",
    tipsData: {
      t1: "Why give?",
      d1: "One single donation can save up to three lives.",
      t2: "After donation",
      d2: "Stay hydrated and rest for 15 minutes.",
      t3: "Eligibility",
      d3: "Check conditions before you travel.",
    },
    bloodGroup: "Group",
    available: "AVAILABLE",
    nearby: "Nearby",
    launchAlertSubtitle: "Urgent blood donation • Needed now",
    bagsRequired: "bags required",
    noUrgentAlertsSubtitle: "Thanks to donors!",
    noUrgentAlerts: "No urgent alerts right now.",
    helpAndAwareness: "Help & Awareness",
    discoverTips: "Discover all our tips",
    myDonations: "MY DONATIONS",
    totalCompleted: "Total completed",
    nextDispo: "NEXT DISPO",
    availableNow: "Available now",
    alertsSent: "ALERTS SENT",
    thisWeek: "This week",
    upcomingRDV: "UPCOMING RDV",
    programmed: "Programmed",
    myBloodGroup: "My blood type",
    addIt: "Add it",
    unavailable: "Unavailable",
    donations: "Donations",
    lastDonation: "Last donation",
    tipsSection: {
      title: "Help & Awareness",
      subtitle: "Discover the impact of your donation",
      didYouKnow: "Did you know?",
      didYouKnowText: "One person in two will need blood during their lifetime",
      stats: {
        frequency: "Max frequency",
        perDonation: "Per donation",
        duration: "Avg duration",
      },
      tips: {
        heart: {
          title: "Heart Health",
          desc: "Blood donation reduces cardiovascular risks",
        },
        hydration: {
          title: "Stay Hydrated",
          desc: "Drink water before and after donation",
        },
        safety: {
          title: "Health Safety",
          desc: "100% safe and sterile process",
        },
        impact: {
          title: "Social Impact",
          desc: "Save up to 3 lives per donation",
        },
      },
    },
  },
  profile: {
    title: "My Profile",
    edit: "Edit my profile",
    history: "Donation history",
    appointments: "My appointments",
    centers: "Donation centers",
    settings: "Settings",
    notifications: "Notifications",
    language: "Language",
    messages: "Messages",
    availability: "Availability",
    availableStatus: "Available to donate",
    unavailableStatus: "Unavailable",
    availabilityUpdated: "Availability updated",
    logout: "Logout",
    logoutConfirm: "Are you sure you want to log out?",
    defaultUser: "User",
    donations: "Donations",
    donationsSubtitle: "Donations performed",
    alerts: "My requests",
    livesSaved: "Lives",
    rating: "Rating",
    menu: "Menu",
    // New fields for Info section
    informations: "Information",
    phone: "PHONE",
    email: "EMAIL",
    city: "CITY",
    bloodGroup: "BLOOD GROUP",
    memberSince: "MEMBER SINCE",
    recentMember: "Recent member",
    availableToDonate: "Available to donate",
    actions: "Actions",
    donationHistory: "Donation history",
    appointments: "My appointments",
    eligibilityTest: "Eligibility test",
    helpAdvice: "Help & Advice",
  },
  messages: {
    title: "Messages",
    inbox: "Inbox",
    noMessages: "No messages yet",
    typeMessage: "Type a message...",
    send: "Send",
    unread: "unread",
    chatUnavailable: "This donor's messaging is not yet activated.",
  },
  centers: {
    title: "Health Centers",
    searchPlaceholder: "Search center...",
    nearby: "Nearby centers",
    noResults: "No center found",
    bookAppointment: "Book appointment",
    appointmentModal: {
      title: "Appointment",
      info: "Online appointment booking will be available soon for this center.",
      close: "CLOSE",
    },
    address: "Address:",
    phone: "Phone:",
  },
  history: {
    title: "Donation History",
    empty: "No donation recorded",
    createAlert: "Create an alert",
    date: "Date:",
    center: "Center:",
    city: "City:",
    defaultType: "Blood donation",
  },
  appointments: {
    title: "My Appointments",
    empty: "No appointment",
    book: "Book an appointment",
    cancel: "Cancel",
    canceled: "Appointment cancelled",
    cancelError: "Cancellation error",
    cancelGenericError: "Error during cancellation",
    date: "Date:",
    time: "Time:",
    type: "Type:",
    code: "Code:",
    phone: "Phone:",
    status: {
      confirmed: "Confirmed",
      completed: "Completed",
      cancelled: "Cancelled",
      pending: "Pending",
    },
  },
  editProfile: {
    title: "Settings",
    header: "Profile",
    subtitle: "Edit your personal information",
    save: "Save Changes",
    success: "Profile updated successfully",
    error: "Error updating profile",
    loadError: "Error loading profile",
    notFound: "Unable to load profile",
    image: {
      pick: "Change Photo",
      error: "Image upload failed",
      success: "Photo updated",
    },
    back: "BACK",
    fields: {
      lastName: "Display Name",
      firstName: "Name",
      phone: "Phone",
      city: "City",
    },
    placeholders: {
      lastName: "e.g. Smith",
      firstName: "e.g. John",
      phone: "e.g. 6XXXXXXXX",
      city: "New York",
    },
  },
  notifications: {
    title: "Notifications",
    channels: "Notification channels",
    push: {
      title: "Push Notifications",
      desc: "Receive alerts on your phone",
    },
    email: {
      title: "Emails",
      desc: "Receive summaries by email",
    },
    sms: {
      title: "SMS",
      desc: "For extremely urgent alerts",
    },
    preferences: "Alert preferences",
    urgentOnly: {
      title: "Urgent only",
      desc: "Do not be notified for normal needs",
    },
    commitment:
      "VitaSang is committed to sending you only relevant notifications related to your blood group and geographical area.",
  },
  login: {
    title: "VitaSang",
    subtitle: "Login to your account",
    forgotPassword: "Forgot password?",
    submit: "LOGIN",
    noAccount: "No account?",
    registerLink: "Register here",
    error: "An unexpected error occurred.",
    fields: {
      phone: "Phone",
      password: "Password",
    },
    placeholders: {
      phone: "e.g. 651234567",
      password: "Your password",
    },
  },
  register: {
    title: "VitaSang",
    subtitle: "Create your donor profile",
    joinUs: "Join us",
    createProfile: "Create your donor profile",
    bloodGroup: "Blood Group",
    vitalInfo: "This information is vital",
    continue: "CONTINUE",
    bloodGroupQuestion: "What is your blood group?",
    unknownBloodGroup: "I don't know my blood group",
    alreadyHaveAccount: "Already registered?",
    signIn: "Sign in",
    step1: {
      title: "Account Info",
    },
    step2: {
      title: "Donor Info",
      subtitle: "Your blood group helps us find the right alerts for you.",
    },
    next: "Next",
    bloodGroupUnknown: "I don't know my blood group",
    back: "Back",
    submit: "CREATE MY ACCOUNT",
    alreadyRegistered: "Already registered?",
    loginLink: "Login here",
    error: "An unexpected error occurred.",
    hintPhone: "Required format: 9 digits (e.g. 651234567)",
    fields: {
      lastName: "Display Name",
      firstName: "Name",
      phone: "Phone",
      password: "Password",
      confirmPassword: "Confirm password",
      bloodGroup: "Blood group",
    },
    placeholders: {
      lastName: "e.g. Smith",
      firstName: "e.g. John",
      phone: "e.g. 651234567",
      password: "Min 6 chars, uppercase, digit",
      confirmPassword: "Confirm your password",
    },
  },
  alert: {
    title: "New Alert",
    submit: "BROADCAST ALERT",
    sosGeneral: "GENERAL SOS (Unknown)",
    donorFound: "{{count}} {{group}} donors found within 10km.",
    searchingDonors: "Searching for compatible donors...",
    locationError: "Location permission denied or unavailable",
    idError: "Error: Alert ID not received",
    genericError: "An error occurred while broadcasting.",
    itineraryUnavailable: "Directions unavailable",
    sending: "Sending...",
    authRequired: "Login required",
    authMessage:
      "Create an account to track your alert and receive notifications.",
    fields: {
      patientName: "Patient Name",
      contactPhone: "Contact Phone",
      location: "Location",
      quantity: "Required quantity (units)",
      urgency: "Urgency level",
      description: "Description (optional)",
    },
    placeholders: {
      location: "e.g. Central Hospital",
      quantity: "e.g. 5",
      description: "Additional details",
    },
    urgencyLevels: {
      NORMAL: "NORMAL",
      URGENT: "URGENT",
      TRES_URGENT: "VERY URGENT",
    },
    response: {
      title: "Blood Emergency",
      details: "Alert Details",
      distance: "{{distance}} km away",
      confirmTitle: "Can you help?",
      confirmDesc:
        "Your blood group is compatible. If you accept, your contact info will be shared with the requester.",
      yes: "I'm coming",
      no: "Not now",
      success: "Thank you! The requester has been notified.",
      contactInfo: "Requester Contact Info",
      chatInitiator: "Chat with the requester",
      initiator: "Requester",
      eligibility: {
        title: "Self-Eligibility Check",
        subtitle: "Please answer honestly for your own safety.",
        warning:
          "If you answer YES to any of these questions, it is better not to donate today.",
        confirmBtn: "I CONFIRM MY ELIGIBILITY",
        testTitle: "Eligibility Test",
        resultTitle: "Test Result",
        eligible: "You seem eligible!",
        notEligible: "A delay might be necessary",
        eligibleDesc:
          "Based on your answers, you meet the basic conditions to donate blood today.",
        notEligibleDesc:
          "Some of your answers indicate it's better to wait or consult a doctor before donating.",
        noteTitle: "Important",
        noteText:
          "This test is indicative. Only the donation center doctor can definitively validate your suitability after a confidential interview.",
        stepText: "Question {{current}} of {{total}}",
        questions: {
          q1: {
            text: "Do you weigh more than 50 kg?",
            hint: "The volume of blood collected depends on your total weight.",
          },
          q2: {
            text: "Are you between 18 and 70 years old?",
            hint: "This is the legal age range for blood donation.",
          },
          q3: {
            text: "Have you eaten and are you well hydrated?",
            hint: "Never donate on an empty stomach to avoid discomfort.",
          },
          q4: {
            text: "Have you had a fever in the last 2 weeks?",
            hint: "A recent infection can be transmitted through blood.",
          },
          q5: {
            text: "Have you had a tattoo or piercing in the last 4 months?",
            hint: "This is a precautionary period for infectious risks.",
          },
          q6: {
            text: "Have you taken antibiotics in the last 7 days?",
            hint: "You must wait until the end of the treatment.",
          },
        },
      },
    },
    tabs: {
      myAlerts: "My Requests",
      myResponses: "My Responses",
    },
    status: {
      en_attente_validation: "Pending",
      en_cours: "In progress",
      resolu: "Resolved",
      annule: "Cancelled",
    },
    tracking: {
      title: "Alert Tracking",
      unknownStatus: "Unknown status",
      launchedOn: "Launched on {{date}}",
      notifiedDonors: "Notified Donors",
      donorIndex: "Donor {{index}}",
      notificationDetails: "Notification Details",
      backHome: "Back to Home",
      stats: {
        notified: "Notified",
        read: "Read",
        accepted: "Accepted",
      },
    },
    actions: {
      accept: "Accept",
      ignore: "Ignore",
      call: "Call",
      close: "Close",
      share: "Share",
      itinerary: "Itinerary",
    },
    empty: {
      sent: "No alerts launched",
      accepted: "No accepted interventions",
      subSent: "Press 'Launch an alert' from home",
      subAccepted: "Alerts you respond to will appear here",
    },
    confirmation: {
      title: "Alert launched!",
      message: "Your alert is being sent to nearby donors...",
      loading: "Searching for donors...",
      info: "You will be redirected to your alert tracking in a few seconds.",
      bypass: "Skip waiting",
    },
    shareMessage:
      "🚨 *VITAL EMERGENCY — BLOOD DONATION APPEAL* 🩸\n\nUrgent need for blood group *{{group}}* to save a life.\n\n📍 *Location* : {{location}}\n🗺️ *Directions* : https://www.google.com/maps/search/?api=1&query={{latitude}},{{longitude}}\n⚠️ *Urgency* : {{urgency}}\n📦 *Need* : {{quantity}} bag(s)\n📞 *Contact* : {{phone}}\n\nThis is for a free and voluntary donation. If you can help or know someone who can, click here:\n🔗 https://vitasangs.netlify.app/alerte/{{id}}\n\n_Please share widely in your groups. Every share can save a life. God bless us._ 🙏🇨🇲\n#VitaSang #Cameroon #VoluntaryBloodDonation",
    fatigue: {
      noAlerts: "You haven't received any alerts today",
      isFatigued: "You're receiving many alerts. Take a break!",
      highEngagement: "Thanks for your commitment! 🎉",
      regularParticipation: "You participate regularly. Great job!",
      explore: "Explore available alerts",
    },
    accountCreationTitle: "Create your account",
    accountCreationMessage:
      "Your alert has been saved successfully. To launch it to nearby donors, you need to create an account. Your account will be created immediately and your alert will be launched automatically.",
    step1: "Create your account with your information",
    step2: "Your alert will be launched automatically",
    step3: "Nearby donors will receive your alert",
    continueToRegister: "Create my account",
    haveAccount: "Already have an account?",
  },
  helpAndAdvice: {
    headerTitle: "Help & Awareness",
    heroTitle: "Why Donate Blood?",
    heroText:
      "Each donation can save up to 3 lives. By donating your blood, you participate in a vital chain of solidarity.",
    statDays: "days between\neach donation",
    statMl: "ml collected\nper donation",
    statLivesSaved: "lives saved\nper donation",
    sectionAdvantagesTitle: "Benefits of Donation",
    sectionAdvantagesItem1: "Reduces the risk of cardiovascular diseases",
    sectionAdvantagesItem2: "Balances iron levels in the blood",
    sectionAdvantagesItem3: "Improves blood circulation",
    sectionAdvantagesItem4: "Stimulates the production of new blood cells",
    sectionAdvantagesItem5: "Early detection of health problems",
    sectionPreparationTitle: "Preparation for Donation",
    sectionPreparationItem1: "Drink plenty of water 48h before",
    sectionPreparationItem2: "Eat healthy and balanced meals",
    sectionPreparationItem3: "Avoid alcohol 24h before",
    sectionPreparationItem4: "Get enough sleep (7-8h)",
    sectionPreparationItem5: "Eat iron-rich foods",
    sectionAfterDonationTitle: "After Donation",
    sectionAfterDonationItem1: "Rest for 10-15 minutes at the clinic",
    sectionAfterDonationItem2: "Drink plenty of water (minimum 1.5L)",
    sectionAfterDonationItem3: "Eat a light snack",
    sectionAfterDonationItem4: "Avoid intense exercise for 24h",
    sectionAfterDonationItem5: "Eat iron-rich foods",
    sectionWhoCanDonateTitle: "Who Can Donate?",
    sectionWhoCanDonateItem1: "18-65 years old (or more depending on health)",
    sectionWhoCanDonateItem2: "Minimum weight: 50 kg",
    sectionWhoCanDonateItem3: "Good general health",
    sectionWhoCanDonateItem4: "Not pregnant",
    sectionWhoCanDonateItem5: "No transmissible diseases",
    faqTitle: "Frequently Asked Questions",
    faqQuestion1: "Is it painful?",
    faqAnswer1:
      "No, the initial prick is minor. After a few seconds, you won't feel anything.",
    faqQuestion2: "How long does a donation take?",
    faqAnswer2:
      "Generally, the donation takes between 8 and 10 minutes. Allow 30 minutes in total including reception and rest.",
    faqQuestion3: "Can you donate if you have tattoos?",
    faqAnswer3:
      "Yes, if the tattoos are more than 4 months old. Otherwise, you must wait 4 months after to donate.",
    faqQuestion4: "What are the risks?",
    faqAnswer4:
      "The risks are very minimal. Sterile needles reduce the risk of infection to almost zero.",
    infoBanner:
      "For more information, consult your local blood transfusion center.",
  },
  guestAlert: {
    title: "SOS Emergency",
    subtitle: "Launch an immediate alert without an account.",
    patientName: "Patient Name",
    contactPhone: "Contact Phone",
    groupNeeded: "Blood Group Required",
    hospital: "Hospital / Location",
    description: "Emergency description",
    submit: "LAUNCH EMERGENCY ALERT",
    success: "Alert sent! An agent will contact you shortly.",
    error: "Error sending the alert.",
    loginLink: "Back to login",
    placeholders: {
      patientName: "e.g. John Doe",
      contactPhone: "e.g. 651234567",
      hospital: "e.g. Laquintinie Hospital",
      description: "e.g. Need 2 units after accident",
    },
    validation: {
      required: "is required",
      phoneFormat: "Format: 6XXXXXXXX",
    },
  },
  alertPublic: {
    invalidLink: "Invalid link.",
    loadError: "Unable to load this alert.",
    thankYou: "Thank you for your commitment!",
    responseError: "Error while responding.",
    loading: "Loading alert...",
    notFound: "Alert not found",
    notFoundDesc: "This alert does not exist or the link has expired.",
    backHome: "Back to home",
    resolved: "Alert resolved",
    cancelled: "Alert cancelled",
    resolvedDesc: "Thank you! The blood need has been met thanks to donors like you.",
    cancelledDesc: "This alert has been cancelled by the requester.",
    seeActive: "See active alerts",
    heroTitle: "Thank you, Hero!",
    heroDesc: "The requester has been notified of your arrival. Head to the location as soon as possible.",
    seeMissions: "See my missions",
    headerTitle: "BLOOD DONATION CALL",
    forPatient: "For:",
    need: "Need:",
    bags: "bag(s)",
    securityNote: "Your donation can save 3 lives today.",
    chat: "Chat",
    refuse: "Refuse",
    loginHint: "You will need to log in to confirm your donation.",
  },
  alertResponse: {
    missionInProgress: "MISSION IN PROGRESS",
    distanceFromYou: "{{distance}} from you",
    unknownPatient: "Unknown patient",
    goingText: "I'M GOING",
    requesterContact: "REQUESTER CONTACT INFO",
    emergencyContact: "Emergency Contact",
    call: "CALL",
    openGPS: "Open GPS",
    location: "Location",
    notFound: "Alert not found",
    notFoundDesc: "This alert may have been resolved or deleted.",
    notified: "Notified",
    accepted: "Accepted",
    read: "Seen",
  },
  alertTracking: {
    confirmTitle: "Donation confirmation",
    confirmMessage: "Do you confirm you made this donation? Your heroic gesture will be recorded.",
    confirmYes: "Yes, I confirm",
    congratsTitle: "Congratulations",
    congratsMessage: "Thank you for your donation! You saved a life.",
    realEngagements: "Real engagements",
    waitingDonors: "Waiting for donors...",
    inProgress: "IN PROGRESS...",
    donationDone: "I MADE THE DONATION",
  },
  alertes: {
    myActivities: "My Activities",
    alertsSent: "alerts sent",
    noAlertHere: "No alert here",
  },
  createAlert: {
    headerTitle: "LAUNCH SOS",
    introText: "Encourage the community to donate for your urgent case.",
    locating: "Getting your location...",
    bloodGroupRequired: "BLOOD GROUP REQUIRED",
    urgencyLevel: "URGENCY LEVEL",
    pleaseWait: "PLEASE WAIT...",
  },
  onboarding: {
    slide1: {
      title: "Find nearby donors",
      description:
        "Connect with compatible blood donors in your area in case of emergency.",
    },
    slide2: {
      title: "Track your donations",
      description:
        "Keep a history of your donations and receive reminders for your next life-saving opportunity.",
    },
    slide3: {
      title: "Receive urgent alerts",
      description:
        "Stay informed in real-time about urgent blood needs and act quickly.",
    },
    actions: {
      emergency: "Emergency: Launch an alert",
    },
  },
};
