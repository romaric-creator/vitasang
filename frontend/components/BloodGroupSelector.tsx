...
export const BloodGroupSelector: React.FC<BloodGroupSelectorProps> = ({
  ...
  return (
    <View style={formStyles.field}>
      <Text style={formStyles.label}>{t("common.bloodGroup")}</Text>
      <View style={[styles.grid, isError && { borderColor: color.error }]}>
        {BLOOD_GROUPS.map((group) => (
          <TouchableOpacity
            key={group}
            style={[styles.option, value === group && styles.selectedOption]}
            onPress={() => onSelect(group)}
            activeOpacity={0.7}
          >
            <Text
              style={[styles.label, value === group && styles.selectedLabel]}
            >
              {group}
            </Text>
          </TouchableOpacity>
        ...
        {/* Unified button: "Je ne connais pas" with SOS icon for alerts */}
        <TouchableOpacity
          style={[
            isAlert ? styles.sosOption : styles.unknownOption,
            (value === UNKNOWN_VALUE || value === "" || !value) &&
              (isAlert
                ? styles.selectedSOSOption
                : styles.selectedUnknownOption),
          ]}
