...
export async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    try {
      // Debug config
      console.log('Constants.expoConfig structure:', JSON.stringify(Constants.expoConfig, null, 2));
      ...
      if (!projectId) {
        console.warn('⚠️ Aucun Project ID EAS trouvé. Les notifications push ne fonctionneront pas sans configuration EAS.');
        console.warn('👉 Exécutez "npx eas project:init" pour lier ce projet à votre compte Expo.');
        return;
      }
